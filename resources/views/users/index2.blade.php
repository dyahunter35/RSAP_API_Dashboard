<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Users') }}
            @can('manage users')
                <x-link href="{{ route('users.create') }}" class="float-right">Add new user</x-link>
            @endcan
        </h2>
    </x-slot>
            ...
            <div class="w-full overflow-x-auto">
                ...
                <div
                    ...
                    <span class="flex items-center col-span-3">
                        Showing
                        {{ $i }}-{{ $i + $users->perPage() - 1 > $users->total() ? $users->total() : $i + $users->perPage() - 1 }}
                        of {{ $users->total() }}
                    </span>
