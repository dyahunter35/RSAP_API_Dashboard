<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Patients') }}
            @can('manage patients')
                <x-link href="{{ route('patients.create') }}" class="float-right">Add new Patient</x-link>
            @endcan
        </h2>
    </x-slot>
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
            <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            ...
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center space-x-4 text-sm float-right">
                                        <x-link href="{{ route('patients.show', $patient) }}">
                                            <button
                                        ...
                                        <x-link href="{{ route('patients.edit', $patient) }}">
                                            <button
                                                class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                                aria-label="Edit">
                                                <svg class="w-5 h-5" aria-hidden="true" fill="white"
                        ...
                        @empty
                            <td class="px-4 py-3 text-xs" colspan="3">
                                <span
                                    class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">
                                    No Data Available
                                </span>
            ...
            <div
                ...
                <span class="flex items-center col-span-3">
                    Showing
                    {{ $i }}-{{ $i + $patients->perPage() - 1 > $patients->total() ? $patients->total() : $i + $patients->perPage() - 1 }}
                    of {{ $patients->total() }}
